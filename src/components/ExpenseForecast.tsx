...
export function ExpenseForecast({ transactions, forecastMonths = 3 }: ExpenseForecastProps) {
  ...
  const { categories, forecastData } = useMemo(() => {
    ...
    categoryMap.forEach((transactions, categoryName) => {
      ...
      // Calculate average
      const monthlyAverage = monthlyData.reduce((sum, data) => sum + data.total, 0) / Math.max(monthlyData.length, 1);
    ...
    const forecast = [];
    const totalMonthlyAverage = categoryData.reduce((sum, category) => sum + category.monthlyAverage, 0);
    const overallTrend = categoryData.reduce((sum, category) => sum + category.trend * category.monthlyAverage, 0) / totalMonthlyAverage;
    ...
    for (let i = 0; i < forecastMonths; i++) {
      ...
      categoryData.forEach(category => {
        monthData[category.name] = category.monthlyAverage * (1 + category.trend * i);
      });
  ...
  const categoryColors = useMemo(() => {
    const colorSet = [
      '#3b82f6', '#ef4444', '#f97316', '#a855f7', '#ec4899', 
      '#14b8a6', '#84cc16', '#eab308', '#06b6d4', '#8b5cf6'
    ];
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button 
            className={`px-3 py-1 rounded text-sm ${viewMode === 'category' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}
        ...
        <ResponsiveContainer width="100%" height="100%">
          {viewMode === 'total' ? (
            <BarChart data={forecastData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              />
              <Legend />
              <Bar dataKey="totalForecast" name="Projected Expenses" fill="#3b82f6" />
            </BarChart>
          ) : (
            <BarChart data={forecastData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" />
      ...
      <div className="mt-6">
        <h3 className="text-md font-medium text-gray-700 mb-3">Top Expense Categories</h3>
              ...
              <div className="flex justify-between items-center">
                ...
                <div 
                  className={`flex items-center ${
                    category.trend > 0 ? 'text-red-500' : category.trend < 0 ? 'text-green-500' : 'text-gray-500'
                  }`}
                >
                  {category.trend > 0 ? (
                  ...
                  <span className="text-xs font-medium">
                    {category.trend !== 0 ? `${Math.abs(category.trend * 100).toFixed(1)}%` : 'Stable'}
                  </span>
                </div>
              </div>
